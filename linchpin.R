###########################################
## Code compares MCMC and Linchpin variable sampler
...
chain <- BLRmcmc(T = 1e5, y = y, X = X, a = a, b = b, start = c(lm.obj$coeff, s2.start), h1 = 2, h2 = .25, h3 = 100)
...
###########################################
## Linchpin variable sampler for 
## Bayesian linear regression for the cars dataset
###########################################

# For the same dataset, we can now implement Monte Carlo sampling
p <- dim(X)[2]
N <- 1e4
A <- t(X)%*%X + diag(p)
A.inv <- solve(A)
...
sig2 <- 1 / rgamma(N, shape = n/2 + a, rate = rate.sig)    # sampling sigma2
...
for(i in 1:N)
{
  beta[i,] <- A.inv %*% t(X) %*%y + Ainv.sqrt %*% rnorm(p, sd = sqrt(sig2))   # Getting beta estimates
}


# Comparing Linchpin sampler vs Accept-reject
par(mfrow = c(1,3))
plot(density(sig2), main = expression(sigma^2))
lines(density(chain$s2), col = "red")

plot(density(beta[,1] ), main = expression(beta[1]))
lines(density(chain$beta[,1]), col = "red")

plot(density(beta[,2] ), main = expression(beta[2]))
lines(density(chain$beta[,2]), col = "red")

# Quite close, but MCMC is not as good and linchpin variable iid samplers are faster@
